...
import {
  activeTaskActions,
  activeTaskSelectors,
} from '../../state/ducks/activeTask';
import { userNameSelectors } from '../../state/ducks/userName';
import { esc, redCross } from '../../variables';
...
const ItemBord = () => {
  ...
  const currentBord = useAppSelector(
    bordsSelectors.selectBord(activeTask.indexBord || ''),
  );
  const dispatch = useAppDispatch();

  const currentTask = currentBord!.tasks.find(
    (task: ITask) => task.id === activeTask.indexTask,
  );
